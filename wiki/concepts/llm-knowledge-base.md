...
sources: [karpathy-llm-knowledge-base.md, aiedge-claude-obsidian-guide.md, nick-spisak-second-brain.md]
updated: 2026-05-11
---
...
```
Layer 1: Raw Sources（ソース）   → 読み取り専用・変更禁止
                ↓ Ingest
Layer 2: Wiki（構造化知識）      → LLM が管理・更新
                ↓ Query
Layer 3: Schema（CLAUDE.md）     → LLM への指示書・構造定義
```

> **Query の出力**: 回答は `outputs/` に保存、または Wiki に反映して複利で蓄積。
...
> AI が誤った内容を wiki に書き込み、それを保存すると次の回答がその誤りの上に構築される。  
> **月次ヘルスチェックで定期的に修正することが必須**。
...
- [[concepts/agent-browser]] — raw/ の自動充填（Vercel Labs CLI）
